...
describe("reactivateCompletedSubagentSession", () => {
  ...
  it("terminates an accepted run when its owner replacement cannot persist", async () => {
    ...
    expect(terminateAcceptedCollectorRunMock).toHaveBeenCalledWith({
      childSessionKey,
      gatewayRunId: "run-next",
    });
  });

  it("terminates an accepted run when another replacement owns the child session", async () => {
    ...
    expect(terminateAcceptedCollectorRunMock).toHaveBeenCalledWith({
      childSessionKey,
      gatewayRunId: "run-losing-successor",
    });
  });
  ...
  it("keeps an accepted run that already owns the child session", async () => {
    ...
    expect(terminateAcceptedCollectorRunMock).not.toHaveBeenCalled();
  });
});
